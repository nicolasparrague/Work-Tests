...
export default class HomeTreatmentCostCalculator extends OmniscriptBaseMixin(LightningElement) {
   ...
   @track
    ...
    isModalGlobal = false;
    isModalCostCalcAttentisComm = false;
    isModalCostCalcAttentisMed = false;
    ...
    truvenLoginWithToken() {
      let input = {
         ...
         planType: this.brandDemography
       };
   
       // Call Member_TreatmentCostCalculator IP
       this.callIP("Member_TreatmentCostCalculator", input).then((data) => {
          if(data.IPResult){
            let dataIP = data.IPResult;
            
            if (dataIP.securityToken && dataIP.endpoint) {
               let params = "?PFWKAT=" + dataIP.securityToken + "&ENTITY_ID=" + dataIP.truvenEntityId + "&MEMBER_ID=" + dataIP.memberId + "&PAYER_ID=" + dataIP.payerId + "&PLAN_TYPE=" + dataIP.truvenPlanType;
               window.open(dataIP.endpoint + params, "_blank");
            }
         }else{
            console.error(`${this.errorMessage} Member_TreatmentCostCalculator`)
         }
       });
    ...
    buidURL() {
        ...
        this.claimEstimatorRedirect = `${protocol}//${hostUrl}/${type}/s/claim-estimate-form`;
    }
