...
import sldsTemplate from "./memberCustomInputField_slds.html";
import ndsTemplate from "./memberCustomInputField_nds.html";
import { inputMask,formatCurrency,loadScript,javascriptNumberFormatter } from "omnistudio/utility";
import { inputLabels as translatedLabels } from "omnistudio/salesforceUtils";
import { lwcPropertyNameConversion } from "omnistudio/utility";
let inputIdGenerator = 0;

export default class MemberCustomInputField extends LightningElement {
    // @api _extraLabelClass;
    // @api _extraValueClass;

    // @api 
    // get extraLabelClass() {
    //     return this._extraLabelClass;
    // }
    // set extraLabelClass(val) {
    //     this._extraLabelClass = val;
    // }

    // get labelClass() {
    //     // let classValue = "nds-checkbox__label";
    //     const useTheme = this.theme ? this.theme : "slds";
    //     return (
    //         ` ${useTheme}-checkbox__label ` +
    //         (this.extraclass ? this.extraclass : "")
    //     );
    // }

    // get valueClass() {
    //     // let classValue = "vlocity-input nds-input_mask";
    //     const useTheme = this.theme ? this.theme : "slds";
    //     return (
    //         ` vlocity-input ${useTheme}-input_mask ` +
    //         (this.extraclass ? this.extraclass : "")
    //     );
    // }
    ...
    renderedCallback() {
        ...
        if (input && this.firstRender) {
            ["change", "blur", "focus"].forEach(event => {
                input.addEventListener(event, this.triggerEvent);
            });

            if (this.mask) {
                ["keydown", "keyup"].forEach(event => {
                    input.addEventListener(event, this.triggerKeyEvents);
                });
            }
    ...
    disconnectedCallback() {
        ...
        if (input) {
            ["change", "blur", "focus"].forEach(event => {
                input.removeEventListener(event, this.triggerEvent);
            });
            if (this.mask) {
                ["keydown", "keyup"].forEach(event => {
                    input.removeEventListener(event, this.triggerKeyEvents);
                });
            }
        }
    }
    ...
    validateValue(value) {
        ...
        if (this.isNumber) {
            return Number.parseFloat(
                JSON.stringify(this.inputValue || "0").replace(/[^0-9.-]/g, "")
    ...
    setInputClass() {
        let _inputClass = `vlocity-input OneLinkNoTx ${this.theme}-input`;
